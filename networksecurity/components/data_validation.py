...
class DataValidation:
    ...
    @staticmethod # Static method avoids unnecessary object initialization for utility functions
    ...
    def validate_numbers_of_columns(self,dataframe:pd.DataFrame)-> bool:
        try:
            numbers_of_columns=len(self.schema_config)
            logging.info(f"Required numbers of columns:{numbers_of_columns}")
            logging.info(f"Data frame has columns:{len(dataframe.columns)}")
            if len(dataframe.columns)==numbers_of_columns:
                return True
            return False
        except Exception as e:
            raise NetworkSecurityException(e,sys)
    ...
    def detect_dataset_drift(self,base_df,current_df,threshold=0.05)->bool:
        try:
            ...
            for column in base_df.columns:
                ...
                is_same_dist=ks_2samp(d1,d2) #Kolmogorov–Smirnov (KS) statistical
                if threshold <=is_same_dist.pvalue: #KstestResult(statistic=0.48, pvalue=0.0012)
                    is_found=False
                else:
                    is_found=True
                    status=False
    ...
    def initiate_data_validation(self)->DataValidationArtifact:
        try:
            ...
            ##vlaidate numbes of columns

            status=self.validate_numbers_of_columns(dataframe=train_dataframe)
            if not status:
                error_message+=f"{error_message} Train datafframe does not contains all the columns.\n"
            ...
            #Data drift = When new incoming data becomes statistically different from training data.
